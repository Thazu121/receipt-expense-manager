...
export default function CameraCard() {
    ...
    try {
      ...
      canvas.toBlob(
        async (blob) => {
          ...
          const result = await dispatch(scanReceipt(file));

          if (scanReceipt.rejected.match(result)) {
            dispatch(setError(result.payload || "Scan failed"));
          }
        },
        "image/png",
        0.95
  ...
  return (
    ...
    >
      {error && (
        <div className="mb-4 p-4 rounded-xl bg-red-500/10 text-red-500 text-center text-sm">
          <p className="font-semibold">{error}</p>

          {isPermissionError && (
            <p className="mt-2 text-xs text-yellow-500">
              Click the lock icon near the address bar → Site
              settings → Camera → Allow, then refresh the page.
            </p>
          )}
        </div>
      )}
      ...
      <div className="relative rounded-2xl overflow-hidden border border-green-500/30 bg-black">
        ...
        {scanning && (
          <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center gap-4 text-white px-6">
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-3 sm:gap-6 text-xs sm:text-sm text-green-500">
        <span>✔ OCR READY</span>
